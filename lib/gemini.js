...
export async function generateDietPlan(userData) {
  ...
  const localDate = userData.local_date || new Date().toISOString().slice(0, 10);
  const userTimezone = userData.user_timezone || "Asia/Kolkata";
...
Every meal must include scheduled_date as "${localDate}" and scheduled_time as a clear AM/PM time string in the user's timezone (${userTimezone}). Do not use ISO timestamps for scheduled_time.
